...
| [Chat render order and subscription parity plan](chat-render-order-subscription.md) | Proposed | Stable timeline item ordering and selected active-thread live attach parity with Codex TUI | Plan for fixing stale lower-seq live merges, active-thread attach behavior, reconnect stability, and agent-browser H1-H4 verification. |
...
| [Gateway queue persistence plan](gateway-queue-persistence.md) | Proposed | Gateway-owned persistent per-thread composer queue for same-gateway multi-client use | Concrete plan for queued-input API, SQLite persistence, drainer, frontend migration, and desktop/iPad agent-browser scenarios. |
