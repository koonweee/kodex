...
- `GET /v1/mcp/servers`, `GET /v1/mcp/servers/{server}/resources/read`, `POST /v1/mcp/servers/{server}/oauth-login`, and `POST /v1/mcp/reload` for read-only app-server MCP inventory, listed-resource reads, explicit OAuth login URL generation, and runtime MCP reload. These routes do not edit MCP server config or call MCP tools.
