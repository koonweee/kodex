...
- `POST /v1/threads/{threadId}/input` for browser composer submits. The gateway owns the TUI-aligned routing decision: use the in-memory thread session view to steer an active turn, clear stale active state and start a new turn when app-server reports no active turn, or create a durable queue row for non-steerable active turns.
