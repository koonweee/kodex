...
- Streaming: SSE for gateway events and selected-thread live projection; WebSocket for interactive PTY terminal I/O.
...
- `GET /v1/terminals`, `POST /v1/terminals`, `DELETE /v1/terminals/{terminalId}`, and `GET /v1/terminals/{terminalId}/ws` for local gateway-host terminal sessions. HTTP routes are generated into OpenAPI; the websocket carries framed PTY stdin, resize messages, and raw output bytes.
...
- `GET /v1/threads/{threadId}/app-surface`, `GET /v1/app-surfaces/{sessionId}/document?revision=...`, and `POST /v1/app-surfaces/{sessionId}/bridge` for the active thread app surface. App surfaces render sandboxed HTML from MCP Apps resources or generated-provider resources in the shared pane. Bridge requests are JSON-RPC-shaped, include a gateway-issued per-revision bridge token held by the web host, and route through the gateway for granted actions such as `ui/message`, `resources/read`, and `tools/call`; ungranted resources/tools are rejected by the gateway. MCP-provider tool calls stay scoped to same-server app grants. Generated-provider MCP tool calls require a pending approval on first request and execute only after retrying with the resolved approval id. Legacy generated-UI routes and MCP tools have been removed; generated app surfaces must use the MCP Apps protocol.
...
- `GET /v1/notifications/status`, `GET/DELETE /v1/notifications/subscription/current?endpoint=...`, `POST /v1/notifications/subscriptions`, `DELETE /v1/notifications/subscriptions/{subscriptionId}`, and `POST /v1/notifications/test` for optional Web Push subscription management. Current-device state is reconciled by the browser's actual push endpoint, and delivery attempts are recorded durably for retry and diagnostics. Push payloads are minimal unread-agent-message or test notices.
- `PATCH /v1/threads/{threadId}/notifications` for gateway-owned per-thread notification preferences. Normal threads default to notifications enabled; disabling a thread suppresses future Web Push delivery for that thread until re-enabled.
...
- `GET /v1/mcp/servers`, `GET /v1/mcp/configured-servers`, `POST /v1/mcp/servers`, `POST /v1/mcp/servers/{server}/replace`, `PATCH /v1/mcp/servers/{server}/enabled`, `DELETE /v1/mcp/servers/{server}`, `GET /v1/mcp/servers/{server}/resources/read`, `POST /v1/mcp/servers/{server}/oauth-login`, and `POST /v1/mcp/reload` for app-server MCP runtime inventory, global MCP config management, listed-resource reads, explicit OAuth login URL generation, and runtime MCP reload. Raw MCP tool calls are not exposed as a general Preferences action; app-surface `tools/call` requests go through the app-surface bridge and stored grants.
- `GET /v1/self-control/status`, `POST /v1/self-control/project-previews/apply`, `POST /v1/self-control/threads`, `POST /v1/self-control/threads/{threadId}/input`, self-control app-surface routes under `/v1/self-control/threads/{threadId}/app-surface`, and self-control automation routes under `/v1/self-control/automations` for guarded agent-facing Kodex Control mutations. MCP tools should use these routes instead of raw CRUD routes so provenance and safety policy stay gateway-owned.
...
The MCP subcommand reads `KODEX_GATEWAY_URL`, defaulting to `http://127.0.0.1:8787`, and refuses non-loopback gateway URLs unless `KODEX_ALLOW_REMOTE_SELF_CONTROL=1` is set. It exposes tools for status, preview apply, thread input, generated app surfaces, and automation management, plus resources such as `kodex://status`, `kodex://projects`, `kodex://threads/{threadId}/app-surface`, and `kodex://automations`.

Generated app-surface tools let Codex open or update a temporary thread-bound HTML pane when direct interaction is clearer than chat. Use `open_app_surface` and `update_app_surface` for generated app surfaces. App-surface HTML should be responsive for desktop split and mobile full-height sheet, include meaningful fallback content, and request bridge grants explicitly. Local UI interactions can stay inside the iframe for embedded-data behavior such as modals, tabs, filters, drilldowns, chart toggles, unit switches, and view changes. Use the bridge `ui/message` method only when an action needs Codex, tools, persistence, workflow continuation, or an explicit user decision. MCP tool/resource bridge calls are routed through gateway-owned grants and inherit the localhost/trusted-VPN security model; generated-provider MCP tool calls additionally require user approval before execution. External network access remains deny-by-default unless a future grant/CSP policy explicitly allows it.
...
The Vite dev server runs on `127.0.0.1:5173` and proxies `/v1` plus `/openapi.json` to the default gateway at `127.0.0.1:8787`. To proxy to another local gateway port during development, set `VITE_KODEX_PROXY_TARGET`. To bypass the Vite proxy and call another gateway origin directly, set `VITE_KODEX_API_BASE_URL`. App surfaces require a sandbox proxy on a different browser origin; loopback development swaps `localhost` and `127.0.0.1` automatically, and same-machine HTTP access through another hostname falls back to `127.0.0.1` on the same port. Remote browsers and HTTPS/non-loopback deployments must set `VITE_KODEX_APP_SURFACE_SANDBOX_URL` to the app-surface sandbox HTML on a distinct origin.
...
`KODEX_NOTIFICATIONS_RECHECK_DELAY_MS` defaults to `2000`; the gateway waits that long through a durable notification delivery row, checks the thread's notification preference, rereads the thread for title/body metadata, and sends Web Push for normal enabled threads even if another client already marked the completed turn seen. Existing subagent and memory-consolidation thread suppression still wins. Temporary push failures are retried without resending to subscriptions that already accepted the delivery; stale push endpoints are disabled only when the push service reports the endpoint is gone.

The web client no longer treats browser storage as notification truth. Preferences > Notifications reads the browser's current `PushSubscription.endpoint`, asks the gateway whether that endpoint is active, and re-upserts it when Enable is pressed. Use the Test button in that panel, or `POST /v1/notifications/test`, to verify delivery without waiting for an agent turn. If a device is registered but silent, inspect the local SQLite `notification_deliveries` and `push_subscriptions` tables to distinguish missing subscriptions, retryable failures, stale endpoints, and OS/browser-level notification suppression.

Device push requires a secure browser context; localhost works for development, while phone/tablet testing over a tailnet or trusted VPN needs HTTPS termination in front of the local gateway. This does not change the MVP access model: keep the gateway on localhost or a trusted VPN, not the public internet.
